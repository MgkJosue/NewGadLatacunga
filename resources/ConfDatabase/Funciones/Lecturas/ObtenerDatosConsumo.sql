...
BEGIN
    ...
    WITH fecha_actual AS (
        SELECT 
            EXTRACT(YEAR FROM fecha_consulta)::INTEGER AS anio_actual,
            EXTRACT(MONTH FROM fecha_consulta)::INTEGER AS mes_actual
    ),
    ultimos_registros AS (
        SELECT 
            apl.numcuenta,
            apl.consumo AS consumo_apl,
            apl.lecturaanterior,
            apl.lectura,
            apl.anio,
            apl.mes,
            ROW_NUMBER() OVER (PARTITION BY apl.numcuenta ORDER BY 
                apl.anio DESC, apl.mes DESC
            ) as rn
        FROM aapplectura apl, fecha_actual fa
        WHERE (apl.anio < fa.anio_actual) 
            OR (apl.anio = fa.anio_actual AND apl.mes <= fa.mes_actual)
    ),
    promedios AS (
        SELECT 
            numcuenta,
            ROUND(AVG(consumo_apl), 2)::NUMERIC(10,2) as promedio_consumo,
            ROUND(AVG(consumo_apl) + rango_unidades, 2)::NUMERIC(10,2) as rango_superior,
            ROUND(AVG(consumo_apl) - rango_unidades, 2)::NUMERIC(10,2) as rango_inferior
    ...
    diferencia_lecturas AS (
        SELECT 
            amv.cuenta,
            amv.abonado,
            CAST(amv.lectura AS INTEGER) as lectura_movillectura,
            ur.lectura as lectura_aplectura,
            CAST(amv.lectura AS INTEGER) - ur.lectura as diferencia,
            amv.coordenadasXYZ,
            amv.observacion AS observacion_movil,
            amv.motivo,
            amv.imagen
        FROM aappMovilLectura amv
        JOIN (
            SELECT numcuenta, lectura
            FROM ultimos_registros
            WHERE rn = 1
        ) ur ON amv.cuenta = ur.numcuenta
    ...
    SELECT 
        ...
        p.promedio_consumo::NUMERIC(10,2) AS promedio,
        CASE 
        ...
        p.rango_superior::NUMERIC,
        p.rango_inferior::NUMERIC,
        dl.lectura_aplectura::INTEGER,
        GREATEST(dl.diferencia, 0)::NUMERIC,
    ...
    WHERE dl.cuenta IS NOT NULL 
      AND ac.no_medidor IS NOT NULL 
      AND ac.clave IS NOT NULL 
      AND dl.abonado IS NOT NULL 
      AND ac.ruta IS NOT NULL
    ORDER BY dl.cuenta
    LIMIT CASE 
        WHEN limite_registros IS NOT NULL THEN limite_registros
        ELSE NULL
    END;
END;
$$ LANGUAGE plpgsql;
